// One-off image optimizer: resize long edge to <=2000px, JPEG q80, strip metadata.
...
// Map each backed-up original to its live destination path.
const targets = [
  { src: `${ORIG}/profile.jpg`, dest: 'public/profile.jpg' },
];
...
for (const { src, dest } of targets) {
  ...
  // Never bloat: if re-encoding grew the file, keep the original bytes.
  if (outSize >= beforeSize) {
    await unlink(tmp);
    const { copyFile } = await import('node:fs/promises');
    await copyFile(src, dest); // keep the smaller original bytes
    outSize = beforeSize;
  } else {
